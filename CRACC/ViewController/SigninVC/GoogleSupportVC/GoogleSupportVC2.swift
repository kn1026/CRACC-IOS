...
class GoogleSupportVC2: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        datePickerView.addTarget(self, action: #selector(GoogleSupportVC2.datePickerValueChanged), for: UIControl.Event.valueChanged)
    }
    ...
    @IBAction func signUpBtnPressed(_ sender: Any) {
        ...
        if let birthday = birthdayTxtField.text, birthday != "", let gender = self.gender, gender != "", let nickname = nicknameTxtField.text, nickname != "" {
                ...
                if currentAge > 13 {
                    ...
                    DataService.instance.checkNickNameUserRef.child(nickname).observeSingleEvent(of: .value, with: { (snapShot) in
                        ...
                        if snapShot.value is NSNull {
                            ...
                            if let name = self.name, name != "", let email = self.email, email != "", let avatarUrl = self.avatarUrl, avatarUrl != "" {
                                ...
                                self.performSegue(withIdentifier: "GoBackToMapVC4", sender: nil)
            ...
            SwiftLoader.hide()
            self.showErrorAlert("Oops !!!", msg: "CRACC: Please choose your birthday")
